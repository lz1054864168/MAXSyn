...
def synthesis_A_B(imageA_path, imageB, save, dim, objects_dir, an=False):
    # 读取图像 A 和图像 B
    ...
    # imageB = cv2.imread('./modelnet_style/background.jpg')

    # maskA = generate_segmentation_mask('./modelnet_style/modelnet_style_1_fake_B.png', False, dim)
    ...
    return resultB, objects_dir

def get_xml(idx, resultB, save, objects_dir):
    cls_id = {'Gun': 0, 'knife': 1, 'wrench': 2, 'pliers': 3, 'scissors': 4, 'hammer': 5, 'fork': 6, 'exploder': 7,
              'firecracker': 8, 'dynamite': 9, 'fire': 10}
    ...
    for ob in objects_dir:
        box = [ob['bndbox']['xmin'],ob['bndbox']['xmax'],ob['bndbox']['ymin'],ob['bndbox']['ymax'] ]
        bb = convert(size, box)
        fp = str(cls_id[ob['name']]) + " " + " ".join([str(a) for a in bb]) + '\n'
        out_file.write(fp)
...
if __name__ =='__main__':
    ...
    dim = {'Gun': (256, 256), 'knife': (384, 384), 'wrench': (256, 256), 'pliers': (256, 256), 'scissors': (256, 256), 'hammer': (256, 256), 'fork': (256, 256), 'exploder': (128, 128), 'firecracker': (384, 384), 'dynamite': (256, 256), 'fire': 10}
    idx = 0
    cls_1 = ['knife', 'scissors', 'fork']
    cls_2 = ['wrench', 'pliers', 'hammer']
    cls_ex = ['exploder', 'firecracker', 'dynamite', 'Gun']
    cls_3 = ['Gun', 'wrench', 'pliers', 'scissors']

    cls_now = cls_3 # choose prohibited item

    path = [os.path.join(comband_path, cls_now[i]) for i in range(len(cls_now))]
    files = [os.listdir(p) for p in path]
    filesp = files
    i = 0
    nums = 7790 # Number of images to be synthesised
    while(i <= nums):
        files = filesp
        if len(cls_ex) > 3:
            numbers = [0, 1, 2, 3]
            num_to_select = random.choice([1, 2, 3, 4])  # Randomise the choice of 3 or 4
            selected_numbers = random.sample(numbers, num_to_select)
            files_T = [files[i] for i in selected_numbers]
            files = files_T
        objects_dir = []
        random.shuffle(files)
        targets = [os.path.join(comband_path, files[i][0].split('_')[0], random.choice(files[i])) for i in range(len(files))]
        for b in os.listdir(backgroud):
            if i > nums:
                break
            bp = os.path.join(backgroud, b)
            imageB = cv2.imread(bp)
            for p in targets:
                imageB, objects_dir = synthesis_A_B(p, imageB, save, dim, objects_dir, an=True)
                if not isinstance(imageB, np.ndarray):
                    objects_dir = []
                    break
            if objects_dir != [] and i <= nums:
                get_xml(idx, imageB, save, objects_dir)
                objects_dir = []
                i += 1
                idx += 1
