...
# Adtion
# 加入了FFL优化G、W模型，FFL对生成图像与真实图像，生成图像的边缘与真实图像的边缘进行代价计算
# 效果一般
...
class AtmeModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'G_L1', 'D_real', 'D_fake', 'G_E', 'G_FFL', 'G_FFL_E']
    ...
    def backward_G(self):
        ...
        self.loss_G_FFL_E = self.FFLoss(self.E, self.fake_B_edge) * self.opt.lambda_L1
        # combine loss and calculate gradients
        self.loss_G = self.loss_G_GAN + self.loss_G_L1 + self.loss_G_E + self.loss_G_FFL_E
