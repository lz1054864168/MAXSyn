...
# best
# Adtion
# 加入了FFL优化G、W模型，FFL对生成图像与真实图像，L1优化生成图像的边缘与真实图像的边缘进行代价计算
...
class AtmeModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
            ...
            # checkpoint = torch.load(r'D:\LZ_project\atme-main\models\facades_BA\latest_net_G.pth',
            #                         map_location=str(self.device))
            checkpoint = torch.load(r'D:\LZ_project\atme-main\checkpoints\X-pix2D-L1forE-FFLforG\latest_net_G.pth',
                                    map_location=str(self.device))
            ...
            #                         map_location=str(self.device))
            checkpoint = torch.load(r'D:\LZ_project\atme-main\checkpoints\X-pix2D-L1forE-FFLforG\latest_net_D.pth',
                                    map_location=str(self.device))
            checkpoint = {'module.' + k: v for k, v in checkpoint.items()}
            ...
            #                         map_location=str(self.device))
            checkpoint = torch.load(r'D:\LZ_project\atme-main\checkpoints\X-pix2D-L1forE-FFLforG\latest_net_W.pth',
                                    map_location=str(self.device))
            checkpoint = {'module.' + k: v for k, v in checkpoint.items()}
